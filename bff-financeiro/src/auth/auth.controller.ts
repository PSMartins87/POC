...
export class AuthController {
  ...
  async iniciarLogin(@Req() req: Request, @Res() res: Response) {
    ...
    return new Promise<void>((resolve, reject) => {
      req.session.save((err) => {
        if (err) {
          this.logger.error({ msg: 'Erro ao salvar contexto PKCE no Redis', err });
  ...
  async keycloakCallback(
    ...
    if (!pkceData || pkceData.state !== state) {
      this.logger.warn({ msg: 'Tentativa de login com state inválido ou sessão expirada.' });
    ...
    try {
      ...
      const UAParserClass = (UAParser as any).default || UAParser;
      const parser = new UAParserClass(req.headers['user-agent'] as string);
      ...
      if (payloadDecodificado?.exp) {
        ...
        ]).catch(err => this.logger.error({ msg: 'Falha ao registrar sessão no Redis', err }));
      }
      ...
      this.logger.error({ msg: 'Falha no fluxo de callback', err: error?.response?.data || error.message });
  ...
  async fazerLogout(@Req() req: Request, @Res() res: Response) {
    ...
    if (sessionId && userId) {
      ...
      this.kafkaClient.emit('security.events', {
        ...
        userId,
        sessionId,
        origin: this.configService.get<string>('APP_NAME'),
      } as SecurityEvent);

      await this.redis.zRem('sessions:expiration_index', `${sessionId}:${userId}`)
        .catch(err => this.logger.error({ msg: 'Falha ao remover sessão no logout', err }));
    }

    req.session.destroy(() => {
      res.clearCookie('financeiro.sid', {
