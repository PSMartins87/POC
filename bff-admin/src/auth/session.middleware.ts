  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      if (this.isTokenExpired(session.accessToken)) {
        this.logger.log('Access Token expirado. Renovando automaticamente via Refresh Token...');

        if (!session.refreshToken) {
          this.logger.warn('Refresh Token ausente na sessão.');
          return next();
        }
        ...
        if (newTokens) {
          ...
          try {
            ...
            this.logger.error('Erro ao atualizar índice de expiração no Redis', redisErr);
          }
          ...
          this.logger.log('Tokens renovados com sucesso!');
        }
      }
      ...
      this.logger.error(`Falha na renovação do token: ${error.message}`);
